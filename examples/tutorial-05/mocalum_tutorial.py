...
def _average_array(a, no_lscans):
    shape = a.shape
    return a.reshape(int(shape[0]/(no_lscans*shape[1])),
                     (no_lscans*shape[1]), shape[1]).mean(axis=1)


def average_sonic(ds, no_lscans):
    ws = _average_array(ds.ws.values, no_lscans)
    wdir = _average_array(ds.wdir.values, no_lscans)
    u = _average_array(ds.u.values, no_lscans)
    v = _average_array(ds.v.values, no_lscans)
    w = _average_array(ds.w.values, no_lscans)

    shape = u.shape

    ds = xr.Dataset({'ws': (['scan', 'point'], ws),
